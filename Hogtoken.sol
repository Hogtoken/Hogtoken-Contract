...
contract Hogtoken is IERC20, Ownable {
    ...
    uint256 private constant _tTotal = _initialSupply * 10**_decimals;
    uint256 private _rTotal = (MAX - (MAX % _tTotal));
    uint256 private _tFeeTotal;

    string private constant _name = 'Hogtoken';
    string private constant _symbol = 'HOG';
    uint8 private constant _decimals = 9;
    uint256 private constant _initialSupply = 21000000;
    ...
    bool private _currentlySwapping = false;
    bool public _provideLiquidity = true;
    ...
    function changeReflectionFee(uint8 newFee) external onlyOwner {
        require(_reflectionFee != newFee, "No change.");
        _reflectionFee = newFee;
        emit ReflectionFeeChanged(msg.sender, newFee);
    }

    function changeLiquidityFee(uint8 newFee) external onlyOwner {
        require(_liquidityFee != newFee, "No change.");
        _liquidityFee = newFee;
        emit LiquidityFeeChanged(msg.sender, newFee);
    }

    function changeMarketingFee(uint8 newFee) external onlyOwner {
        require(_marketingFee != newFee, "No change.");
    ...
    function changeFeeExclusion(address account, bool takeFee) external onlyOwner {
        ...
        _isExcludedFee[account] = takeFee;
    }

    function changeSwapBreakpoint(uint256 swappoint) external onlyOwner {
        require(_initSwapBreakpoint != swappoint, "No change.");
        _initSwapBreakpoint = swappoint;
    }
    ...
    function toggleMarketingEthMode(bool newState) external onlyOwner {
        require(_marketingEthMode != newState, "No change");
        _marketingEthMode = newState;
    }

    function withdraw() external onlyOwner {
        require(!_currentlySwapping, "Swap in progress");
    ...
    function _transfer (address sender, address recipient, uint256 amount) private {
        ...
        if (_initSwapBreakpoint <= contractTokenBalance && !_currentlySwapping && sender != _uniswapV2Pair && _provideLiquidity) {
            _addLiquidity(_initSwapBreakpoint);
        }

        bool takeFee = !(_isExcludedFee[sender] || _isExcludedFee[recipient]);
        if (takeFee) {
    ...
    function _addLiquidity(uint256 tokenAmount) private InitSwap {
        uint256[2] memory halves;
        halves[0] = tokenAmount / 2;
        halves[1] = tokenAmount - halves[0];

        uint256 initialBalance = address(this).balance;
        _swapHogToEth(halves[0]);
        
        uint256 addedBalance = address(this).balance - initialBalance;
        _insertLiquidity(halves[1], addedBalance);
    }

    function _swapHogToEth(uint256 tokenAmount) private {
    ...
    function _insertLiquidity(uint256 tokenAmount, uint256 ethAmount) private {
        _approve(address(this), address(_uniswapRouter), tokenAmount);

        _uniswapRouter.addLiquidityETH{ value: ethAmount }(address(this), tokenAmount, 0, 0, owner(), block.timestamp);
    }

    function _removeReflectFees() private {
        if (_reflectionFee == 0)
            return;
    ...
    function _restoreReflectFees() private {
